...
export const chromeTabsMovePromise = (
  tabId: number,
  windowId: number,
  index: number,
): Promise<ChromeTab[]> =>
...
const chromeTabsGroupPromise: GroupPromise = (
...
export const applyRulesForTab = (tab: ChromeTab, config: RulesConfig) => {
  ...
  const chromeTabsQueryPromise: QueryPromise = (
    url?: string,
  ): Promise<ChromeTab[]> =>
    new Promise(function (resolve, reject) {
      const query = {
...
export const moveTabsPromise =
  (
    chromeTabsMovePromise: (
      tabId: number,
      windowId: number,
      index: number,
    ) => Promise<ChromeTab[]>,
  ...
  (tabs: ChromeTab[]) => {
    ...
    const referenceWindowId = [...windowCounts.entries()].sort(
      (a, b) => b[1] - a[1],
    )[0][0];
    ...
    const movePromises = tabs
      ...
      .map(
        (t, index) => () =>
          chromeTabsMovePromise(
            t.id!,
            referenceTab.windowId,
            referenceTab.index + index,
          ),
      );
    return promiseSerial(movePromises).then(() => Promise.resolve(tabs));
  };
    ...
    queryPromise()
      .then((allTabs) => {
        const tabs = allTabs.filter(
          (t) => t.id !== undefined && t.url == tab.url && t.id != tab.id,
        );
        console.log(
          tabs.length,
          "identical tabs to tab id ",
          tab.id,
          " : ",
          tabs,
        );
